...
coor_file, coor, missed_oa = "us-ca-alameda_county.CSV", Hash.new, 0
...
content = Hash.new #temporary testing, no looping
  ...
  File.open(filename+".js", 'w') do |f|
    ...
    CSV.foreach(csv_file, headers: true ) do |row|
      ...
      if address == last_address
        ...
        content[:weight] += net_value
      else
        p content[:weight], content[:weight].class unless content.empty?
        content[:weight] = (content[:weight]**0.5).to_i  unless content.empty? #maybe logcan used
        f.puts content.to_json.gsub(':"new google',': new google').gsub(')","weight"','),"weight"').gsub('"location"', 'location'). gsub('"weight"', ' weight') + ","  unless content.empty? #quatation marks can't be used in javascript
        #content = {lat: bldg_coordinates.first, lng: bldg_coordinates.last, weight: net_value}
        p content = {location: "new google.maps.LatLng("+bldg_coordinates.first+","+bldg_coordinates.last+")", weight: net_value}
