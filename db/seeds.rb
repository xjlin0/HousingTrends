...
def create_realestate(candidates, net_value, county)
  ...
  updated_value = realestate.send(current_year_value) + net_value #some bldg have many units
  ...
  p realestate.send( (current_year_value+'=').to_sym, updated_value )
  p realestate.save!
end
...
f.write Time.now.to_s + " finished loading of postal abbreviations hash\n"
...
coor_files.each do |coor_file|
  ...
  CSV.foreach(coor_file, headers: true ) do |address|
    ...
    parsed_address.street = address["STREET"].split.last if parsed_address.street.nil? #for cases of "850 Avenue H 94130", but such cases are not yet found in SFC open data
    num_street = Chronic::Numerizer.numerize(parsed_address.street).to_i
  ...
  f.write Time.now.to_s + " finished seeding of #{coor_file}, missed addresss: " + missed_oa.to_s + "\n"
...
csv_files.each do |csv_file|
  ...
  f.write Time.now.to_s + " finished parsing the file: #{csv_file}. missed record count: #{missed_tx.length}, missed address:" + missed_tx.to_s + "\n"
  f.write " Totally #{g_counts} of records is geocoded by Google\n"
  counter += 1
end
...
f.write Time.now.to_s + " finished linear regression\n"
...
f.write Time.now.to_s + " finished trend score calculation\n"
...
f.write Time.now.to_s + " finished avarage calculation\n"
