...
coor_files, missed_oa = ['db/us-ca-alameda_county6.CSV','db/us-ca-san_francisco0.CSV'], Array.new
coor_files.each do |coor_file|
  CSV.foreach(coor_file, headers: true ) do |address|
    ...
    address_line = address["NUMBER"] +" "+address["STREET"]
    ...
    current_geocode.update_attributes(lat: address["LAT"].to_f, lng: address["LON"].to_f) if address["LAT"] && address["LON"]  #to_f required?
    current_geocode.update_attributes(zip: parsed_address.zipcode.to_i) if parsed_address.zipcode
  end
end
puts "finished seeding of alameda and SF county geocoder, missed addresss: " + missed_oa.to_s
puts "finished seeding of alameda and SF county geocoder, the count of missed addresss: " + missed_oa.length.to_s
...
counter, missed_tx = 12, Array.new  #Alameda's data starting from 2012
...
csv_files.each do |csv_file|
  ...
  CSV.foreach('./db/'+csv_file, headers: true ) do |row|
    ...
    if candidates.empty?  #Check for ordinal  730 29TH ST (accessor) => Fourth Street (opengeocoder)
      p "\n entering number and ordinal \n"
      num_street = Chronic::Numerizer.numerize(parsed_address.street).to_i
      parsed_address.street = I18n.with_locale(:en) { num_street.to_words ordinal: true, remove_hyphen: true } if num_street > 0
      # MLK, JFK abbreviations convertion or other geocoding processing goes here.
      # M L KING JR WAY(accessor) ==>  Martin Luther King Junior Way (open geocoder)
      parsed_address.street = "Martin Luther King Junior" if parsed_address.street.include?("M L King Jr")
      p candidates = Opengeocoder.where(street_address: parsed_address.line1.chomp!('.'), zip: parsed_address.zipcode)
    end
    p "\n line 73 \n"
    if candidates.empty? # check for google geocoder
      p "\n entering google geocoder \n"
      sleep 1.1  #google map api limit
      p location = GoogleGeocoder.geocode(address)  #Google can handle strange address
      p "The above result are from Google\n"
      ...
      p current_geocode = Opengeocoder.find_or_create_by(street_address: parsed_address.line1.chomp!('.'))
      current_geocode.update_attributes(lat: location.lat, lng: location.lng) if location.lat && location.lng
      current_geocode.update_attributes(zip: parsed_address.zipcode.to_i) if parsed_address.zipcode
      candidates = [current_geocode]
    end
    ...
    geocoding_one.lat, geocoding_one.lng = candidate.lat, candidate.lng if geocoding_one.lat.nil?
  ...
  puts "finished parsing the file: #{csv_file}. missed record count " + missed_tx.length.to_s
  counter += 1
end
...
counter, missed_tx = 8, Array.new  #Alameda's data starting from 2012
